...
def getGameRegion():
    ...
    region = pyautogui.locateOnScreen(imPath('1.Start.png'))
    if region is None:
        raise Exception('Could not find game on screen. Is the game visible?')

    # calculate the region of the entire game
    logging.debug('Region found')
    topRightX = region[0] + region[2] # left + width
    topRightY = region[1] # top
    GAME_REGION = (topRightX - 803, topRightY, 803, 601) 
...
def navigateStartGameMenu():
    ...
    while True: # loop because it could be the blue or pink Play button displayed at the moment.
        pos = pyautogui.locateCenterOnScreen(imPath('2.Play.png'), region=GAME_REGION, grayscale=True, confidence= 0.5)
        '''locateOnScreen parameters( image address, region- area to find image, confidence- include negligibe error correction for detecting images, need openCV library,
        grayscale represents that pixels are in RGB values'''
        if pos is not None:
            break
    ...
    # click on Continue
    while True:
        pos= pyautogui.locateCenterOnScreen(imPath('3.Continue.png'),grayscale=True, confidence= 0.5, region= GAME_REGION)
        if pos is not None:
            break
    #pos = pyautogui.locateCenterOnScreen(imPath('Screenshot(14).png'), region=GAME_REGION,grayscale=True, confidence= 0.5)
    pyautogui.click(pos, duration=0.25)
    logging.debug('Clicked on Continue button.')

    # click on Skip
    logging.debug('Looking for Skip button...')
    pos= pyautogui.locateCenterOnScreen(imPath('4.Skip.png'),grayscale=True, confidence= 0.5)
    while True: # loop because it could be the yellow or red Skip button displayed at the moment.
        #pos = pyautogui.locateCenterOnScreen(imPath('Screenshot (16).png'), region=GAME_REGION,grayscale=True, confidence= 0.5)
        if pos is not None:
            break
    pyautogui.click(pos, duration=0.25)
...
def setupCoordinates():
    ...
    INGRED_COORDS = {SHRIMP: (GAME_REGION[0] + 40, GAME_REGION[1] + 405),
                     RICE:   (GAME_REGION[0] + 125, GAME_REGION[1] + 405),
                     NORI:   (GAME_REGION[0] + 40, GAME_REGION[1] + 475),
                     ROE:    (GAME_REGION[0] + 125, GAME_REGION[1] + 475),
                     SALMON: (GAME_REGION[0] + 40, GAME_REGION[1] + 555),
                     UNAGI:  (GAME_REGION[0] + 125, GAME_REGION[1] + 555),} #all are at center of the image
    PHONE_COORDS = (GAME_REGION[0] + 765, GAME_REGION[1] + 440)
    TOPPING_COORDS = (GAME_REGION[0] + 695, GAME_REGION[1] + 340)
    ORDER_BUTTON_COORDS = {SHRIMP: (GAME_REGION[0] + 616, GAME_REGION[1] + 275),
                           UNAGI:  (GAME_REGION[0] + 740, GAME_REGION[1] + 275),
                           NORI:   (GAME_REGION[0] + 616, GAME_REGION[1] + 345),
                           ROE:    (GAME_REGION[0] + 740, GAME_REGION[1] + 345),
                           SALMON: (GAME_REGION[0] + 616, GAME_REGION[1] + 415),}
    RICE1_COORDS = (GAME_REGION[0] + 695, GAME_REGION[1] + 380)
    RICE2_COORDS = (GAME_REGION[0] + 700, GAME_REGION[1] + 370)

    NORMAL_DELIVERY_BUTTON_COORDS = (GAME_REGION[0] + 630, GAME_REGION[1] + 375)

    MAT_COORDS = (GAME_REGION[0] + 245, GAME_REGION[1] + 470)
    ...
    while True:
        ...
        if time.time() - 12 > LAST_GAME_OVER_CHECK:
            ...
            if result == LEVEL_WIN_MESSAGE:
                ...
                pos = pyautogui.locateCenterOnScreen(imPath('continue_button.png'), region=GAME_REGION,grayscale=True, confidence= 0.5)
                pyautogui.click(pos, duration=0.25)
                logging.debug('Clicked on Continue button.')
                pos = pyautogui.locateCenterOnScreen(imPath('continue_button.png'), region=GAME_REGION,grayscale=True, confidence= 0.5)
...
def clickOnPlates():
    ...
    for i in range(6):
        pyautogui.click(100 + GAME_REGION[0] + (i * 130), GAME_REGION[1] + 180)
...
def getOrders():
    ...
    MENU=(GAME_REGION[0]+60,GAME_REGION[1]+70, 670,30)
    #all images for height 30, problem with width of images 

    orders = {}
    for orderType in (ALL_ORDER_TYPES):
        allOrders = pyautogui.locateAllOnScreen(imPath('%s_order.png' % orderType), region=GAME_REGION,grayscale=True, confidence= 0.5)
...
def makeOrder(orderType):
    ...
    while time.time() < ROLLING_COMPLETE and pyautogui.locateOnScreen(imPath('clear_mat.png'), region=(GAME_REGION[0] + 115, GAME_REGION[1] + 295, 220, 175),grayscale=True, confidence= 0.5) is None:
        time.sleep(0.1)
...
def findAndClickPlatesOnBelt():
    ...
    for color in ('pink', 'blue', 'red'):
        result = pyautogui.locateCenterOnScreen(imPath('%s_plate_color.png' % (color)), region=(GAME_REGION[0] + 343, GAME_REGION[1] + 300, 50, 100),grayscale=True, confidence= 0.5)
        if result is not None:
...
def orderIngredient(ingredient):
    ...
    if ingredient == RICE and ORDERING_COMPLETE[RICE] is None:
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_rice.png'), region=(GAME_REGION[0] + 498, GAME_REGION[1] + 242, 90, 75),grayscale=True, confidence= 0.5):
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_%s.png' % (ingredient)), region=(GAME_REGION[0] + 446, GAME_REGION[1] + 187, 180, 180),grayscale=True, confidence= 0.5):
...
def checkForGameOver():
    ...
    result = pyautogui.locateOnScreen(imPath('you_win.png'), region=(GAME_REGION[0] + 188, GAME_REGION[1] + 94, 262, 60),grayscale=True, confidence= 0.5)
    if result is not None:
        pyautogui.click(pyautogui.center(result))
        return LEVEL_WIN_MESSAGE

    # check for "You Fail" message
    result = pyautogui.locateOnScreen(imPath('you_failed.png'), region=(GAME_REGION[0] + 167, GAME_REGION[1] + 133, 314, 39),grayscale=True, confidence= 0.5)
    if result is not None:
